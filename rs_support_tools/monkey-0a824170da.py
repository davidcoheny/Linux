...
EXTERNAL_SCRIPTS = {
    'oom': {
        ...
        'url': 'https://raw.githubusercontent.com/LukeShirnia/out-of-memory/786db070d5e68549ce3ecc18a019c7b31878c252/oom-investigate.py',
    ...
    'mysqltuner': {
        ...
        'url': 'https://raw.githubusercontent.com/major/MySQLTuner-perl/59e5f40ed199e07cae6004d734c5bacf8fff3ece/mysqltuner.pl',
    ...
    'apache2buddy': {
        ...
        'url': 'https://raw.githubusercontent.com/richardforth/apache2buddy/687c9e68b9f650b7ea0dee8a30cd09a01c3a7b2e/apache2buddy.pl',
    ...
    'php_fpmpal': {
        ...
        'url': 'https://raw.githubusercontent.com/pksteyn/php-fpmpal/93c45df179313df59ba660851d97b4f6c406ba0d/php-fpmpal.sh',
    ...
    'traffic_analyzer': {
        ...
        'url': 'https://raw.githubusercontent.com/tahz7/traffic_analyser/92dab3b9c67f0c71bde82e2b9e9502bc84570d3c/traffic_analyser.py',
    ...
    'ps_mem': {
        ...
        'url': 'https://raw.githubusercontent.com/pixelb/ps_mem/9f54e1aa3a87ec176ce8b71f02673e0d8293b344/ps_mem.py',
    ...
    'postfixbuddy': {
        ...
        'url': 'https://raw.githubusercontent.com/dsgnr/postfixbuddy/27210a68c8774659cc05867e7c74f649263faa74/postfixbuddy.py',
    ...
    'disk_usage': {
        ...
        'url': 'https://raw.githubusercontent.com/LukeShirnia/Low_Disk/ce9a25e6e3810041384ae66838de7387c33663ec/disk_usage_check.sh',
...
def bytes2human(bytes, bps=False):
    """
    ...
    for unit in scale:
        ...
        elif abs(bytes) < 1024.0:
            return "%.0f %s" % (round(bytes), unit)
        bytes /= multiplier
    return "%.0f %s" % (round(bytes * multiplier), unit)


def seconds2human(seconds, precision=9):
    """
    ...
    for i in range(len(names) - 1, -1, -1):
        ...
        if a > 0:
            result.append('%d %s' % (a, names[i][1 % a]))
...
class Filesystem(Fact):  # {{{
    ...
    def __init__(self):
        ...
        for (mount, used, total, iused, inodes, error) in self.stats:
            ...
            self._lines.append(fmtstr % mount +
                               '%s of %7s%s' % (color(usage), bytes2human(total), iwarn))
# }}}
...
class Dmesg(Fact):  # {{{
    ...
    DMESG_LINE_RE = re.compile("^\[\s*(?P<time>\d+\.\d+)\](?P<line>.*)$")
    ...
    def _check_driveclient_active(self, pid):
        ...
        try:
            ...
            if ex.errno == 13:  # Errno 13 is permission denied
                return self._lines.append(self._critical("Script is not running as the root user. Please check if escalation failed."))
            raise
        return False
# }}}
...
class ClusterStatus(object):  # {{{
    ...
    def services(self):
        ...
        for group in groups:
            ...
            if group.attributes['state_str'].value == 'started':
                svc['running'] = True
            else:
                svc['running'] = False
            ret.append(svc)
...
class Services(Fact):  # {{{
    ...
    class Service(object):  # {{{
        ...
        def __init__(self, fact, init, pkg, svcname):
            self._fact = fact
            self._init = init
            self._pkg = pkg
    ...
    class Apache(Service):  # {{{
        SERVICE_NAMES = ['httpd', 'apache2']
        _TRY_FILES = ['/etc/httpd/conf/httpd.conf', '/etc/apache2/apache2.conf']
    # }}}

    class Nginx(Service):  # {{{
        SERVICE_NAMES = ['nginx']
        _TRY_FILES = ['/etc/nginx/nginx.conf']
    # }}}

    class MySQL(Service):  # {{{
        SERVICE_NAMES = ['mysql', 'mysqld', 'mariadb']
        _TRY_FILES = ['/usr/share/doc/mysql-server', '/usr/libexec/mysqld', '/usr/sbin/mysqld',
                      '/etc/my.cnf', '/etc/mysql/my.cnf']
    # }}}

    class Redis(Service):  # {{{
        SERVICE_NAMES = ['redis-server', 'redis']
        _TRY_FILES = ['/etc/redis']
    # }}}

    class Lsync(Service): #{{
        SERVICE_NAMES = ['lsyncd']
        _TRY_FILES = ['/etc/lsyncd.conf', '/etc/init.d/lsyncd']
    #}}

    class Webmin(Service):  # {{{
        SERVICE_NAMES = ['webmin']
        _TRY_FILES = ['/usr/share/webmin/', '/etc/webmin']
    # }}}

    class Plesk(Service):  # {{{
        SERVICE_NAMES = ['sw-cp-server']
        _TRY_FILES = ['/etc/nginx']
    ...
    class cPanel(Service):  # {{{
        SERVICE_NAMES = ['cpanel']

        def title(self):
            return self._fact._critical("cPanel")
    # }}}

    _CANDIDATES = [
        Apache, Nginx, MySQL, Redis, Lsync, Webmin, Plesk, Docker, cPanel]
    ...
    def __init__(self, system):
        ...
        if self.is_cluster():
            ...
            for svc in sorted(cluster_services, key=lambda x: not x['local']):  # display local services first
                ...
                if hasattr(self, svc['type']):
                    ...
                    svcobj = cls(self, self._init, self._pkg, svc['name'])
                    title = svcobj.title()
                else:
                    title = svc['type']
        ...
        for cls in candidates:
            for svc_name in cls.SERVICE_NAMES:
                candidate_map[svc_name] = cls

        # intersect
        installed_services = set(system.init.list_services()) & set(candidate_map.keys())
...
def main():
    ...
    ext_scripts_help = ("When invoking an external script that requires arguments, you must "
                        ...
                        "Example (invoke MySQLtuner's help): " + Fact.ORANGE +
                        "monkey.py --mysql -- --help" + Fact.RESET)
    ...
    parser = optparse.OptionParser(usage="monkey [options]", **parser_kwargs)
    ...
    try:
        ...
        if options.nginx:
            try:
                ...
                print "Please report error to on %s" % (
                    "https://github.rackspace.com/IAW/monkeys/issues")
                print "----------------------------------------------------------------------"
        ...
        if typ is not SystemExit:
            import traceback
            print Fact.ORANGE + "Whoops, it looks like the monkey script failed badly :("
            print
            print "If you've got a minute, would you mind raising an issue here:"
            print "https://github.rackspace.com/IAW/monkeys/issues"
            print
